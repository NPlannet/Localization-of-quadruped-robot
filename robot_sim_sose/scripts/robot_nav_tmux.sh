...
Options:
  ...
  --terminal-app <app>    GUI terminal to use: gnome-terminal, mate-terminal,
...
What it does:
  ...
  4. Opens one real GUI terminal per robot stack command.
...
Requirements on your laptop:
  ...
  scp
  sshpass
  one supported GUI terminal app
EOF
}
...
detect_terminal_app() {
  ...
  echo "No supported GUI terminal app found." >&2
  echo "Install gnome-terminal or xterm, or pass --terminal-app <app>." >&2
  exit 1
}
...
terminal_container_command() {
  ...
  case "${role}" in
    xgo-motion|xgo-bridge|lidar|filter|slam|nav2|foxglove)
...
echo
if [ \$status -ne 0 ]; then
  echo "[${role}] Session ended with status \$status."
fi
echo "Local shell stays open."
exec bash -i
EOF
}
...
main() {
  ...
  local roles=("${bridge_role}" "lidar" "filter" "slam" "nav2")
  ...
  echo "Launching robot stack terminals with ${terminal_app}..."
  launch_terminals "${terminal_app}" "${roles[@]}"
  start_cleanup_watcher
