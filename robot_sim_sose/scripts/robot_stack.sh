...
SLAM_PARAMS=${SLAM_PARAMS:-${WORKSPACE}/src/xgo_driver_bridge/config/slam_toolbox_robot.yaml}
...
MAP_OUTPUT_ROOT=${MAP_OUTPUT_ROOT:-${WORKSPACE}/evaluation/results/live/maps}
...
Commands:
  ...
  save-map       Save the current map to evaluation/results/live/maps/xgo_map.
EOF
}
...
case "${1:-}" in
  ...
  save-map)
    ...
    ros2 run nav2_map_server map_saver_cli -f "${MAP_OUTPUT_ROOT}/xgo_map"
    ;;
  *)
    usage
    exit 1
